...
df_table <- data.frame("model" = c("karlinsky", "linear", "who", "negbin", "negbin_lin_trend"),
                       "correction_constant" = c(1.422, 1.437, 5.648, 1.104, 1.511),
...
for (model in df_table$model){
  ...
  c <- df_table$correction_constant[which(df_table$model == model)]
  df_sum$ci_low_c <- df_sum$ed-c*((df_sum$ed - df_sum$ci_low))
  df_sum$ci_high_c <- df_sum$ed+c*((df_sum$ci_high - df_sum$ed))
...
df_table_final <- df_table_freq[,c(1,2,5,6)]
...
write.csv(df_table_final, "./Tables/ed_id_final_correction_europe.csv", row.names = F)


# US ----------------------------------------------------------------------

df_table <- data.frame("model" = c("karlinsky", "linear", "who", "negbin", "negbin_lin_trend"),
                       "correction_constant" = c(1.38, 1.39, 5.24, 1.013, 1.41),
                       "share_significant_ed" = NA,
                       "share_significant_ed_c" = NA,
                       "share_significant_id" = NA,
                       "share_significant_id_c" = NA)
...
write.csv(df_table_final, "./Tables/ed_id_final_correction_us.csv", row.names = F)

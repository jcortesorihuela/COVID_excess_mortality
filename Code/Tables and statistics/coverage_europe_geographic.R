...
for (j in 1:nrow(df_constant)){
  ...
  files <- list.files(path = "./Output data/Cross-validation data/",
                      pattern = paste0(df_constant$model[j], "_europe_"),
                      full.names = T)
  ...
  units <- unique(df_aux$country_name)
  
  df_constant_unit_all <- c()
  ...
  for (k in 1:length(units)){
    ...
    df <- df_aux[df_aux$country_name == units[k],]
    ...
    df_constant_unit <- data.frame(model = df_constant$model[j],
                                   country_name = units[k],
                                   coverage = mean(df_sum$I),
                                   constant = c,
                                   constant_q = ifelse(mean(df_sum$I_c_q) >= 0.95, p, Inf))
    
    df_constant_unit_all <- rbind(df_constant_unit_all, df_constant_unit)
    
  }
  
  df_constant_model_all <- rbind(df_constant_model_all, df_constant_unit_all)
}

#write.csv(df_constant_model_all, "./Tables/summary_europe_geographic.csv", row.names = F)

...
#### Load in data ####
exp_obs_bymonthyear <- read.csv("./Output data/Death counts data/europe_mortality.csv")
...
for(i in 1:max(exp_obs_bymonthyear$country_num)){
  ...
  pred <- predict(annual_model,
                  se.fit = TRUE,
                  type = "response",
                  newdata = data.frame(year = c(rep(2015 + 5, 12),
                                                rep(2015 + 6, 12)),
                                       month = c(1:12, 1:12)))
...
save(acm_monthly_predictions_tier1, 
     file = "./Output data/Excess death data/acm_monthly_predictions_tier1_europe_ed.RData")
...
load(paste0("./Output data/Excess death data/acm_monthly_predictions_tier1_europe_ed.RData"))
exp_obs_bymonthyear <- read.csv("./Output data/Death counts data/europe_mortality.csv")
colnames(exp_obs_bymonthyear) <- c("iso3", "year", "month", "observed")
...
df_aux <- data.frame("country_name" = acm[,1],
                     year = rep(rep(unique(acm_monthly_predictions_tier1$year), each = 12), 32),
                     "time" = acm[,2],
                     "deaths" = (acm$acm1),
...
write.csv(df_aux, "./Output data/Excess death data/excess_death_who_monthly_europe.csv", 
          row.names = F)

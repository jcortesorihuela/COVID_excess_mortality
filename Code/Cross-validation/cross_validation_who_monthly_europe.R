...
for (k in 1:length(years)){
  ...
  save(acm_monthly_predictions_tier1, 
       file = paste0("./Output data/Cross-validation data/acm_monthly_predictions_tier1_europe_",years[k], ".RData"))
...
for (k in 1:length(years)){
  ...
  load(paste0("./Output data/Cross-validation data/acm_monthly_predictions_tier1_europe_",years[k], ".RData"))
  exp_obs_bymonthyear <- read.csv("./Output data/Death counts data/europe_mortality.csv")
  colnames(exp_obs_bymonthyear) <- c("iso3", "year", "month", "observed")
  ...
  write.csv(df_aux, paste0("./Output data/Cross-validation data/who_europe_", years[k],".csv"), 
            row.names = F)
